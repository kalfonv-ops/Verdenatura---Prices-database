...
def save_snapshot(conn, products, now, period):
    ...
    for p in products:
        ...
        if ref in active or ref in back:
            conn.execute(
                "UPDATE products SET last_seen=?,name=?,sub_name=?,category=?,subcategory=?,grouping=?,size=?,color=? WHERE ref=?",
                [now, p["name"], p.get("sub_name",""), p["category"], p["subcategory"], p["grouping"], p.get("size",""), p.get("color",""), ref])
        else:
            conn.execute(
                "INSERT OR IGNORE INTO products "
                "(ref,name,sub_name,category,subcategory,grouping,price_tiers,size,color,first_seen,last_seen,is_active) "
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,1)",
                [ref, p["name"], p.get("sub_name",""), p["category"], p["subcategory"],
                 p["grouping"], json.dumps(p.get("price_tiers",{})), p.get("size",""), p.get("color",""), now, now])
            new_count += 1
        if p.get("price", 0) > 0:
            conn.execute(
